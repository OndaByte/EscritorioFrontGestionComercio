...
package com.OndaByte.AntartidaFront.vistas.remitos;

import com.OndaByte.AntartidaFront.controladores.RemitoController;
...
public class RemitoPanel extends JPanel {
    ...
    private RemitoController remitoControlador;
    private HashSet<String> permisos;
    private ArrayList<HashMap<String, Object>> remitosDetallado;
    private Remito remitoSeleccionado;

    private RemitoPanel esta;

    private static Logger logger = LogManager.getLogger(RemitoPanel.class.getName());

    static {
        if (logger.isTraceEnabled()) {
            logger.trace("Init logger en RemitoPanel");
        }
    }
    ...
    public RemitoPanel() {
        ...
        remitoControlador = RemitoController.getInstance();
        totalElementos = 0;
        filtro = "";
        filtroEstado = "";
        filtroDesde = "";
        ...
        remitoControlador.filtrar(filtro, filtroDesde, filtroHasta, filtroEstado, "" + pagina, "" + tamPagina, new DatosListener<List<HashMap<String, Object>>>() {
            @Override
            ...
            public void onSuccess(List<HashMap<String, Object>> datos, Paginado p) {
                remitosDetallado = new ArrayList<>(datos);
    ...
    private void addAcciones() {
        ...
        btnEditar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                if(remitoSeleccionado != null ){
                    logger.debug("fecha de pago == null ?" + remitoSeleccionado.getFecha_pago());
                    if(remitoSeleccionado.getFecha_pago()==null){
                        RemitoController.getInstance().buscarRemito(
                        remitoSeleccionado.getId(),
                        new DatosListener<HashMap<String, Object>>() {
                            ...
                            public void onSuccess(HashMap<String, Object> resultado) {
                                ...
                                ArrayList<ItemRemito> items = (ArrayList<ItemRemito>)resultado.get("items");
                                RemitoModal modal = new RemitoModal(MiFrame.getInstance(),remitoSeleccionado, items,c);
        ...
        btnEliminar.addActionListener(e -> {
            if (remitoSeleccionado != null) {
                remitoControlador.eliminarRemito(remitoSeleccionado.getId(), new DatosListener<String>() {
                    @Override
    ...
    private void initTabla() {
        String[] headers = {"Cód:", "Cliente", "Teléfono", "Fecha Emisión", "Fecha Pagado","Nro. Remito", "Total","Observaciones", "Acciones"};
            ...
            if(remi.getFecha_pago()==null){
                panel.agregarBoton("Pagar", IconSVG.ACEPTAR, e -> {
                    ...
                    if (res) {
                        ...
                        remitoControlador.actualizarRemito(aux,
                                new DatosListener() {
                                    @Override
        ...
        tabla.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ...
                if (row >= 0) {
                    remitoSeleccionado = (Remito) remitosDetallado.get(row).get("remito");
                    logger.trace("Remito seleccionado: " + remitoSeleccionado.getId());
                }
            }
        });
    }
    ...
    private List<Object[]> generarData() {
        ...
        for (HashMap<String, Object> map : remitosDetallado) {
            Remito r = (Remito) map.get("remito");
            rows.add(new Object[]{
                    r.getId(),
                    r.getC_nombre(),
                    r.getC_telefono(),
    ...
    private void imprimirRemito(Remito remito) {
        remitoControlador.buscarRemito(
                remito.getId(),
                new DatosListener<HashMap<String, Object>>() {
                    
            @Override
            public void onSuccess(HashMap<String, Object> resultado) {
                Remito remi = (Remito)resultado.get("remito");
//                Orden o = (Orden)resultado.get("orden");
                Cliente c = (Cliente)resultado.get("cliente");
                ArrayList<ItemRemito> itemsRemi = (ArrayList<ItemRemito>)resultado.get("items");
    ...
    public static String generarFilasTabla(List<ItemRemito> items) {
        StringBuilder sb = new StringBuilder();
        for (int i = 0; i < items.size(); i++) {
            ItemRemito ir = items.get(i);
            sb.append("<tr>");
    ...
    private String cargarTemplate() {
        try {
            InputStream is = RemitoModal.class.getClassLoader().getResourceAsStream("templates/template_remito.html");
            BufferedReader br = new BufferedReader(new InputStreamReader(is, StandardCharsets.UTF_8));
            return br.lines().collect(Collectors.joining("\n"));
        } catch (Exception e) {
            logger.error("RemitoModal: ", e.getMessage(), e);
        }
        return "";
    }
}
