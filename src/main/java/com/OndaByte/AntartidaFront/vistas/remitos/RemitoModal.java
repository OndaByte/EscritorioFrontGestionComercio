...
public class RemitoModal extends JDialog {

    private Remito remito;
    private ArrayList<ItemRemito> items;
    ...
    private static Logger logger = LogManager.getLogger(RemitoModal.class.getName());

    public RemitoModal(Frame parent, ArrayList<ItemRemito> items, Integer orden_id, Cliente c) {
        super(parent, "Crear Remito", true);
        this.remito = new Remito();
        this.cliente = c;
        this.items = items;
        this.remito.setOrden_id(orden_id);
        this.editar = false;
        
        remitoController = RemitoController.getInstance();
        initVista();
        setSize(600, 640);
        setLocationRelativeTo(null);
    }
    
    public RemitoModal(Frame parent,Remito r, ArrayList<ItemRemito> items, Cliente c) {
        super(parent, "Editar Remito", true);
        this.remito = r;
        this.cliente = c;
        this.items = items;
        this.editar = true;
    ...
    private JPanel renderPanelTop() {
        ...
        titulo = new JLabel("Remito de: " + cliente.getNombre());
    ...
    private JPanel renderPanelCentro() {
        ...
        textAreaObservaciones.setText(this.remito.getObservaciones());
    ...
    private void validarYGuardar() {
        ...
        if (errores.isEmpty()) {
            Remito nuevoRemito = this.crearRemito();
            if(!editar){
                remitoController.crearRemito(nuevoRemito, items,
                ...
                nuevoRemito.setId(this.remito.getId());
                remitoController.editarRemito(nuevoRemito, items,
                    new DatosListener<String>() {
                        @Override
                        public void onSuccess(String datos) {
                            Dialogos.mostrarExito(datos);
    ...
    private Remito crearRemito() {
        Remito r = new Remito();
        r.setOrden_id(remito.getOrden_id());
