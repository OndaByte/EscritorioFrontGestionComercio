...
package com.OndaByte.MisterFront.vistas.clientes;
...
public class ClientePanel extends JPanel{
    ...
    private ClientePanel esta;
    
    private static Logger logger = LogManager.getLogger(ClientePanel.class.getName());

    static{
        if(logger.isDebugEnabled()){
            logger.debug("Init logger en ClientePanel");
        }
    }
    ...
    public ClientePanel() {
        ...
        clienteControlador = ClienteController.getInstance();
        totalElementos=0;
        filtro="";
        pagina = 1;
        tamPagina = 20;
        clienteControlador.filtrar(filtro, "" + pagina, "" + tamPagina,new DatosListener<List<Cliente>>(){
            ...
            public void onSuccess(List<Cliente> datos, Paginado p) {
                ...
                initTabla(); //tabla
                setVisibleByPermisos(tabla,"CLIENTE_LISTAR");
    ...
    public void initVista() {
        ...
        setVisibleByPermisos(txtBuscar,"CLIENTE_LISTAR");
        if(btnVer==null){
            btnVer = new JButton("Ver detalles");
        }
        setVisibleByPermisos(btnVer,"CLIENTE_LISTAR");
        if(btnNuevo==null){
            btnNuevo = new JButton("Nuevo");
        }
        setVisibleByPermisos(btnNuevo,"CLIENTE_ALTA");
        if(btnEditar==null){
            btnEditar = new JButton("Editar");
        }
//        setVisibleByPermisos(btnEditar,"mostrar_pedido");
        setVisibleByPermisos(btnEditar,"CLIENTE_MODIFICAR");
        if(btnEliminar==null){
    ...
    private void initEstilos() {
        ...
        btnVer.setIcon(new IconSVG(IconSVG.NUEVO,2));
        ...
        MisEstilos.aplicarEstilo(btnVer, MisEstilos.BOTONCITO);
    ...
    private void addAcciones(){
        txtBuscar.getDocument().addDocumentListener(new DocumentListener() {
            public void insertUpdate(DocumentEvent e) { filtrarCliente();}

            public void removeUpdate(DocumentEvent e) {filtrarCliente();}

            public void changedUpdate(DocumentEvent e) {filtrarCliente();}
        ...
        btnEditar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                if(clienteSeleccionado != null){
                    ClienteModal modal = new ClienteModal(MiFrame.getInstance(),clienteSeleccionado);
                    modal.setVisible(true); // bloquea el thread hasta que es cerrado
                    filtro="";
                    pagina = 1;
                    reload();
        ...
        btnVer.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                if(clienteSeleccionado != null){
                    ClienteModal modal = new ClienteModal(MiFrame.getInstance());
                    modal.cargarDatos(clienteSeleccionado, false);
                    modal.setVisible(true); // bloquea el thread hasta que es cerrado
                    filtro="";
                    pagina = 1;
                    reload();
        ...
        btnEliminar.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                if(clienteSeleccionado != null){
                    if(Dialogos.confirmar("¿Está seguro que desea eliminar este cliente?", "Eliminar Cliente")){
                        clienteControlador.eliminarCliente(clienteSeleccionado.getId(),
                                new DatosListener(){
                                    @Override
    ...
    public void initTabla() {
        ...
        tabla.addMouseListener(new java.awt.event.MouseAdapter() {   // listenner
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ...
                logger.trace(row);
                if ( row >= 0 ){
                    clienteSeleccionado = (Cliente) clientes.get(row);
                }
            }
        });
    }
